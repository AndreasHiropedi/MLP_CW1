    ...
    def fprop(self, inputs, stochastic=True):
        ...
        if stochastic:
            if self.share_across_batch:
                same_mask = self.rng.uniform(0,1,size = inputs.shape[1:]) < self.incl_prob
                return inputs * same_mask
            else:
                different_masks = self.rng.uniform(0,1,size = inputs.shape) < self.incl_prob
                return inputs * different_masks
